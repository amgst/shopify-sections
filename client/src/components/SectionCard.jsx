...
export default function SectionCard({ id, title, slug, category, description, thumbnailUrl, downloads, isPremium = false, }) {
    const sectionUrl = createSectionUrl({ id, title, slug });
    
    return (<Card className="group overflow-hidden hover-elevate active-elevate-2 transition-all duration-200 hover:shadow-lg" data-testid={`card-section-${id}`}>
      <Link href={`/section/${sectionUrl}`}>
        <div className="relative overflow-hidden bg-muted">
          <img src={thumbnailUrl} alt={title} className="w-full h-full object-cover transition-transform duration-200 group-hover:scale-105" data-testid={`img-thumbnail-${id}`}/>
          {isPremium && (<Badge className="absolute top-3 right-3 bg-accent text-accent-foreground" data-testid={`badge-premium-${id}`}>
              Premium
            </Badge>)}
        </div>
      </Link>
